...
| 29 | ~~`tiles.yml` produisait un run rouge de 0 s (« workflow file issue ») à chaque push sur `master` depuis le 2026-09-05~~ : GitHub valide le fichier à chaque push et `fromJSON(inputs.boxes)` échoue quand `inputs` est vide (événement `push`), bien que le workflow ne se déclenche qu'en `workflow_dispatch` | — | ✅ résolu 2026-09-06 : `fromJSON(inputs.boxes \|\| '[0,1,2,3,4,5,6,7]')` ; sans effet sur les runs manuels |
