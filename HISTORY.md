...
| 2026-09-19 | Ciel étoilé : un matériau `transparent: true` aurait placé les étoiles dans la passe transparente de three, **après** le globe opaque ; sans test de profondeur elles se seraient dessinées par-dessus la Terre. Vu à la conception, avant tout rendu. | `transparent: false` + `AdditiveBlending` (three applique quand même le mélange) + `renderOrder` −1 ; verrouillé par `stars.test.ts`. |
